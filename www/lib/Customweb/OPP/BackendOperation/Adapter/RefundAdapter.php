 ...
 * @Bean
 */
class Customweb_OPP_BackendOperation_Adapter_RefundAdapter extends Customweb_OPP_BackendOperation_Adapter_AbstractAdapter implements Customweb_Payment_BackendOperation_Adapter_Service_IRefund
{
	...
	public function partialRefund(Customweb_Payment_Authorization_ITransaction $transaction, $items, $close)
	{
		...
		if ($response->result->code != '000.000.000'
			...
			if (!empty($response->result->description)) {
				...
				throw new Exception(Customweb_I18n_Translation::__("The refund failed due to an unkown reason."));
			}
		}
