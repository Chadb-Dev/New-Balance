...
AdminOrder.prototype = {
    ...
    changeAddressField: function(event) {
        ...
        if ((type == 'billing' && this.shippingAsBilling && !this.isShippingMethodReseted) || (type == 'shipping' && !this.shippingAsBilling && !this.isShippingMethodReseted)) {
            ...
            if (checkbox) {
                ...
                if (confLink.readAttribute('disabled')) {
                    ...
                    productConfigure.setConfirmCallback(listType, function() {
                        ...
                        var productPrice = parseFloat(this._calcSimpleProductPrice());
    ...
    _calcProductPrice: function() {
        ...
        var getPriceFields = function(elms) {
            ...
            for (var i = 0; i < elms.length; i++) {
                ...
                } else if (((elms[i].type == 'checkbox' || elms[i].type == 'radio') && elms[i].checked) || ((elms[i].type == 'file' || elms[i].type == 'text' || elms[i].type == 'textarea' || elms[i].type == 'hidden') && Form.Element.getValue(elms[i]))) {
    ...
    _calcSimpleProductPrice: function() {
        var productPrice = 0;
        ...
        var getPriceFields = function(elms) {
            ...
            for (var i = 0; i < elms.length; i++) {
                if (elms[i].type == 'select-one' || elms[i].type == 'select-multiple') {
                    for (var ii = 0; ii < elms[i].options.length; ii++) {
                        if (elms[i].options[ii].selected) {
                            if (elms[i].options[ii].config != undefined) {
                                var nbrallowedProducts = elms[i].options[ii].config.allowedProducts;
                                if (nbrallowedProducts.length < 2) {
                                    var productId = elms[i].options[ii].config.allowedProducts.shift();
                                    if (productId != undefined) {
                                        if (selectedProducts.indexOf(productId) == -1) {
                                            productPrice += parseFloat(ProductConfigure.spConfig.config.childProducts[productId]['finalPrice']);
                                        }
    ...
    removeSidebarItem: function(id, from) {
        this.loadArea(['sidebar_' + from], 'sidebar_data_' + from, {
            remove_item: id,
            from: from
        });
    },
