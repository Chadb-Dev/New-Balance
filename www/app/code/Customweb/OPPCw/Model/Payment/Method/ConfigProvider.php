...
class ConfigProvider implements \Magento\Checkout\Model\ConfigProviderInterface
{
     ...
     * @return string
     */
        public function getForm(AbstractMethod $method)
    {
		
		$arguments = array(
			'method' => $method,
 		);
		return \Customweb_Licensing_OPPCw_License::run('uum4r28s4s8508ck', $this, $arguments);
	}

	final public function call_683efs3i79c8fbln() {
    ...
    protected function getFailureMessage(AbstractMethod $method)
    {
    	if ($this->_checkoutSession->getQuote()->getPayment()->getMethod() == $method->getCode()) {
	    	$failureMessage = $this->_checkoutSession->getOPPCwFailureMessage();
	    	if (!empty($failureMessage)) {
	    		$this->_checkoutSession->unsOPPCwFailureMessage();
    ...
    protected function getImageUrl(AbstractMethod $method)
    {
    	try {
    		...
    		return $this->_assetRepo->getUrlWithParams('Customweb_OPPCw/images/payment/method/' . $method->getPaymentMethodName() . '.png', $params);
    ...
    {
    	return new \Customweb\OPPCw\Model\Renderer\CheckoutForm($method->getCode());
    }
     ...
     */
    protected function getAuthorizationMethodAdapter(AbstractMethod $method)
    {
    	if (!array_key_exists($method->getCode(), $this->authorizationMethods)) {
	    	$context = $this->_authorizationMethodFactory->getContextFactory()->createQuote($method);
