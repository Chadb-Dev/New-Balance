...
class Configuration
{
	...
	{
		$modulesDirectory = $this->_moduleReader->getModuleDir(\Magento\Framework\Module\Dir::MODULE_ETC_DIR, 'Customweb_OPPCw');
		$this->_xmlParser->load($modulesDirectory . '/config.xml');
		$xmlArray = $this->_xmlParser->xmlToArray();
		foreach ($xmlArray['config']['_value']['default']['oppcw']['general'] as $key => $value) {
			if (is_array($value)) {
				$value = $value['_value'];
			}
	...
	{
		$mediaDirectory = $this->_storeManager->getStore()->getBaseMediaDir() . '/customweb/oppcw';
		$mediaUrl = $this->_storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA) . '/customweb/oppcw';
