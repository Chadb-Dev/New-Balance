...
namespace Customweb\OPPCw\Block\Adminhtml\Checkout;
...
class Payment extends \Magento\Backend\Block\Widget\Form\Container
{
	 ...
	 * @var string
	 */
	protected $_blockGroup = 'Customweb_OPPCw';
	...
	protected function _construct()
	{
		...
		$this->setId('oppcw_checkout_payment');

		$this->buttonList->update('save', 'label', __('Submit Order'));
		$this->buttonList->update('save', 'class', 'primary');
		$this->buttonList->update('save', 'id', 'oppcw-payment-submit-button');
		...
		$this->buttonList->add(
			...
			[
				'label' => __('Cancel'),
				'class' => 'cancel',
				'id' => 'oppcw-payment-cancel-button'
			]
		);
	}
	...
	public function getTransaction()
	{
		if (!($this->transaction instanceof \Customweb\OPPCw\Model\Authorization\Transaction)) {
			...
			if (!($transaction instanceof \Customweb\OPPCw\Model\Authorization\Transaction) || !$transaction->getId()) {
