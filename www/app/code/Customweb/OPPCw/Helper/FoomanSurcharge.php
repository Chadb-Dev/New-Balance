...
class FoomanSurcharge extends \Magento\Framework\App\Helper\AbstractHelper
{
	...
	public function getQuoteSurchargeAmount($quoteId)
	{
		...
		$items = $this->_objectManager->get('Fooman\Totals\Model\QuoteAddressTotalManagement')->getByQuoteId($quoteId);
	...
	public function getOrderSurchargeAmount(\Magento\Sales\Model\Order $order)
	{
		...
		$items = $this->_objectManager->get('Fooman\Totals\Model\OrderTotalManagement')->getOrderTotalsByOrderId($order->getId());
		return $this->calculateAmount($items);
	}
	...
	public function getInvoiceSurchargeAmount(\Magento\Sales\Model\Order\Invoice $invoice)
	{
		...
		$items = $this->_objectManager->get('Fooman\Totals\Model\InvoiceTotalManagement')->getInvoiceTotalsByInvoiceId($invoice->getId());
		return $this->calculateAmount($items);
	}
	...
	public function getCreditmemoSurchargeAmount(\Magento\Sales\Model\Order\Creditmemo $creditmemo)
	{
		...
		$items = $this->_objectManager->get('Fooman\Totals\Model\CreditmemoTotalManagement')->getCreditmemoTotalsByCreditmemoId($creditmemo->getId());
		return $this->calculateAmount($items);
	}
