...
namespace Customweb\OPPCw\Plugin\Sales\Controller\Adminhtml\Order\Create;
...
class Save
{
	...
	public function aroundExecute(\Magento\Sales\Controller\Adminhtml\Order\Create\Save $subject, \Closure $proceed)
	{
		...
		$this->_coreRegistry->register('oppcwcheckout_moto', true);
		$result = $proceed();
		if ($this->_messageManager->getMessages()->getLastAddedMessage()->getType() == 'success') {
			$this->_messageManager->getMessages()->clear();
			$order = $this->_coreRegistry->registry('oppcw_checkout_last_order');
			if ($order instanceof \Magento\Sales\Model\Order) {
				if ($order->getPayment()->getMethodInstance() instanceof \Customweb\OPPCw\Model\Payment\Method\AbstractMethod) {
					$transaction = $this->_transactionFactory->create()->loadByOrderId($order->getId());
					$transaction->setSendEmail($sendConfirmation);
					$transaction->save();

					return $this->_redirectResultFactory->create()->setPath('oppcw/checkout/payment', ['order_id' => $order->getId(), 'send_confirmation' => $sendConfirmation]);
