...
//        $force = ($status != REMARKETY_MGCONNECTOR_CALLED_STATUS);
    ...
    public function serializeCustomer($customer){
        ...
        if($customer->getIsSubscribed() || $path == "newsletterpopup/index/subscribe"){
            $allowed = true;
        } else {
            ...
            if($mark_grouped_parent){
                ...
                if(!empty($childProductsIds)) {
                    ...
                    foreach ($childProductsIds as $childProductsId) {
                        $childProd = $this->_rmCore->loadProduct($childProductsId);
                        $childProdPrice = $rules->getRulePrice($now, $store->getWebsiteId(), 0, $childProd->getId());
                        $childProdPrice = empty($childProdPrice) ? $childProd->getFinalPrice() : (float)$childProdPrice;
                        $childProdStockLevel = (int)Mage::getModel('cataloginventory/stock_item')
                            ->loadByProduct($childProd)->getQty();
                        $variants[] = array(
                            'product_id' => $childProd->getId(),
                            'sku' => $childProd->getSku(),
                            'price' => $childProdPrice,
                            'inventory_quantity' => $childProdStockLevel,
                            'created_at' => $childProd->getCreatedAt(),
                            'updated_at' => $childProd->getUpdatedAt()
                        );
                    }
                }
            }
