...
//                $product->setStoreId($mage_store_view_id)->load($product->getId());
                ...
                //get price and inventory for grouped products
                if($mark_grouped_parent && $product->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_GROUPED){
                    //get price and stock level from child products
                    ...
                    if(!empty($childProductsIds)) {
                        ...
                        foreach ($childProductsIds as $childProductsId) {
                            $childProd = $this->loadProduct($childProductsId);
                            if($childProd){
                                //price
                                $childProdPrice = (float)$childProd->getFinalPrice();
                                $minPrice = is_null($minPrice) ? $childProdPrice : min($childProdPrice, $minPrice);
                                //stock level
                                if (key_exists($childProd->getId(), $inventories)) {
                                    $inventory += $inventories[$childProd->getId()];
                                } else {
                                    $childInventory = (int)Mage::getModel('cataloginventory/stock_item')
