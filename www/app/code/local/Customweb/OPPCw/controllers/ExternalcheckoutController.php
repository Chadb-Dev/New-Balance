...
class Customweb_OPPCw_ExternalcheckoutController extends Mage_Core_Controller_Front_Action
{
	...
	public function loginAction()
	{
		...
		$this->getLayout()->getBlock('content')->append(
			$this->getLayout()->createBlock('oppcw/externalCheckout_login')->setContext($context)
		);
		$this->renderLayout();
	}
	...
	public function registerPostAction()
	{
		...
		if ($this->getRequest()->isPost()) {
			...
			if ($dateOfBirth != null) {
				$data['dob'] = Mage::helper('OPPCw/externalCheckout')->getDateAsString($dateOfBirth);
			}
			...
			if (true !== ($result = Mage::helper('OPPCw/externalCheckout')->validateCustomerData($quote, $data, $registerMethod))) {
				if (!is_array($result)) {
					$result = array($result);
				}
		...
		if ($this->getRequest()->isPost()) {
			...
			if (!empty($login['username']) && !empty($login['password'])) {
				try {
					...
					// Mage::helper('OPPCw')->logException($e); // PA DSS violation: this exception log can disclose customer password
				}
			} else {
	...
	/**
	 * @return Customweb_OPPCw_Model_ExternalCheckoutContext
	 */
	private function getContext()
	{
		if ($this->_context == null) {
			$this->_context = Mage::getModel('oppcw/externalCheckoutContext')->loadByQuote($this->getQuote());
		}
