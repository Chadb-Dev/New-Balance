...
class Customweb_OPPCw_Helper_Data extends Mage_Core_Helper_Abstract
{
	private static $container = null;

	public function log($message, $level = null, $file = '', $forceLog = false) {
		if (Mage::getStoreConfig('oppcw/general/debug_log') == '1') {
	...
	public function getAssetResolver() {
		return new Customweb_Asset_Resolver_Composite(array(
			Mage::getModel('oppcw/asset_skinResolver'),
			Mage::getModel('oppcw/asset_jsResolver'),
			Mage::getModel('oppcw/asset_templateResolver'),
			new Customweb_Asset_Resolver_Simple(Mage::getBaseDir('media') . '/customweb/oppcw/assets/', Mage::getBaseUrl('media') . '/customweb/oppcw/assets/')
		));
	}
	...
	{
		$transaction = Mage::getModel('oppcw/transaction')->load($orderPaymentId, 'order_payment_id');
	...
	{
		$transaction = Mage::getModel('oppcw/transaction')->load($orderId, 'order_id');
		if ($transaction !== null && $transaction->getId()) {
			return $transaction;
		}
	...
	{
		$transaction = Mage::getModel('oppcw/transaction')->load($transactionId);
		if ($transaction === null || !$transaction->getId()) {
			return null;
		}
		return $transaction;
	...
	{
		$transaction = Mage::getModel('oppcw/transaction')->load($transactionId, 'transaction_external_id');
		if ($transaction === null || !$transaction->getId()) {
			return null;
		}
		return $transaction;
	...
	{
		$transaction = Mage::getModel('oppcw/transaction')->load($paymentId, 'payment_id');
		if ($transaction === null || !$transaction->getId()) {
			return null;
		}
		return $transaction;
	...
	{
		if ($customer->getData('oppcw_migrated')) {
			return;
		}
		...
		$payments = Mage::getModel('sales/order_payment')->getCollection()
			...
			->addFieldToFilter('method', array('like' => 'oppcw_%'));
		...
		$query = 'UPDATE ' . Mage::getSingleton('core/resource')->getTableName('customer_entity')
			. ' SET oppcw_migrated = 1, created_at = ' . $connection->quote(Varien_Date::formatDate($customer->getCreatedAt()))
	...
	protected function migrateTransaction(Mage_Sales_Model_Order $order)
	{
		if ($order !== null && $order->getPayment() !== false) {
			...
			if (!empty($additionalData)) {
				...
				$alias = Mage::getModel('oppcw/aliasdata')->load($order->getId(), 'order_id');
	...
	public function getPaymentCustomerContext($customerId = null)
	{
		...
		return Customweb_OPPCw_Model_PaymentCustomerContext::getByCustomerId($id);
	}
	...
	{
		$configAdapter = new Customweb_OPPCw_Model_ConfigurationAdapter();
		return $configAdapter->getConfigurationValue($key);
	}

	public function setConfigurationStoreId($storeId)
	{
		Customweb_OPPCw_Model_ConfigurationAdapter::setStoreId($storeId);
	}
	...
	{
		if (Mage::getStoreConfig('oppcw/general/wait_for_success') != '1') {
