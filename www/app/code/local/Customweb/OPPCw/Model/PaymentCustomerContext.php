...
class Customweb_OPPCw_Model_PaymentCustomerContext implements Customweb_Payment_Authorization_IPaymentCustomerContext
{
	...
	{
		$customerContext = Mage::getModel('oppcw/customercontext')->load($this->customerId);
		if ($customerContext->getId() && $customerContext->getContext()) {
			$result = Mage::helper('OPPCw')->unserialize($customerContext->getContext());
	...
	protected function updateContextMapInDatabase(array $map)
	{
		...
		if (!$customerContext->getId()) {
			$customerContext = Mage::getModel('oppcw/customercontext');
			$customerContext->setId($this->customerId);
		}

		$customerContext->setContext(Mage::helper('OPPCw')->serialize($map));
