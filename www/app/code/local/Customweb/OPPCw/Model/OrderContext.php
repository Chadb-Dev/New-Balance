...
class Customweb_OPPCw_Model_OrderContext extends Customweb_Payment_Authorization_OrderContext_AbstractDeprecated implements Customweb_Payment_Authorization_IOrderContext
{
	/**
	 * Transient fields
	 */
	...
	/**
	 *
	 * @param Customweb_OPPCw_Model_Method $paymentMethod
	 * @param unknown $isOrderAvailable
	 */
	public function __construct(Customweb_OPPCw_Model_Method $paymentMethod, $isOrderAvailable, $storeId = null, $quote = null)
	{
		...
		$checkoutId = $session->getOPPCwCheckoutId();
		if($checkoutId === null) {
			$checkoutId = Customweb_Core_Util_Rand::getUuid();
			$session->setOPPCwCheckoutId($checkoutId);
		}
	...
	{
		$orderContext = new Customweb_OPPCw_Model_OrderContext($order->getPayment()
	...
	{
		return Mage::getModel('oppcw/invoiceItems')->getInvoiceItems($this->getOrderQuote(), $this->getStore()->getStoreId(), $this->useBaseCurrency());
	}
