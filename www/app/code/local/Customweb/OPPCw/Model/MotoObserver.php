...
class Customweb_OPPCw_Model_MotoObserver
{
	/**
	...
	public function postOrderSave(Varien_Event_Observer $observer)
	{
		...
		if ($orderId) {
			...
			if (strpos($payment->getMethodInstance()
				->getCode(), 'oppcw') !== false) {
				Mage::helper('OPPCw')->log("Codes match.");
				if ($messageType == 'success') {
					$order->addStatusToHistory(Customweb_OPPCw_Model_Method::OPPCW_STATUS_PENDING, Mage::helper('OPPCw')->__('Payment is pending at OPPCw'));
					$order->save();
					Mage::helper('OPPCw')->log("Redirect to Moto authorization");
					...
					$controller->getResponse()
						->setRedirect(Mage::helper('adminhtml')->getUrl('*/motooppcw/process', array(
	...
	public function beforeOrderSave(Varien_Event_Observer $observer)
	{
		try {
			...
			if (isset($_POST['payment']['method'])) {
				...
				if (strpos($paymentCode, 'oppcw') !== false) {
