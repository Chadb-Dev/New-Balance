...
class Customweb_OPPCw_Model_Observer
{
	private $timeout = 0;

	public function initCart(Varien_Event_Observer $observer)
	{
		if (Mage::getStoreConfig('oppcw/general/cancel_existing_orders')) {
				...
				foreach ($productIds as $productId) {
					...
					if (!$product->isSalable()) {
						foreach ($orders as $order) {
							...
							$order->setIsActive(0);
							$order->addStatusToHistory(Customweb_OPPCw_Model_Method::OPPCW_STATUS_CANCELED, Mage::helper('OPPCw')->__('Order cancelled, because the ordered products are not available.'));
							$order->save();
						}
						break;
					}
	...
	public function placeOrder(Varien_Event_Observer $observer)
	{
		...
		try {
			if (strpos($order->getPayment()->getMethodInstance()->getCode(), 'oppcw') === 0) {
		...
		try {
			if (strpos($order->getPayment()->getMethodInstance()->getCode(), 'oppcw') === 0) {
				...
				if (Mage::registry('cw_is_moto') == null && !empty($sessionTransactionId)) {
					$transaction = Mage::helper('OPPCw')->loadTransaction($sessionTransactionId);
	...
	public function cancelOrder(Varien_Event_Observer $observer)
	{
		$order = $observer->getOrder();
		if (strpos($order->getPayment()->getMethodInstance()->getCode(), 'oppcw') === 0) {
			$order->addStatusHistoryComment(Mage::helper('OPPCw')->__('Transaction cancelled successfully'));
		}
	}
	...
	public function invoiceView(Varien_Event_Observer $observer)
	{
		...
		if (strpos($invoice->getOrder()->getPayment()->getMethodInstance()->getCode(), 'oppcw') === 0) {
			$transaction = Mage::helper('OPPCw')->loadTransactionByOrder($invoice->getOrder()->getId());
			...
			if (Mage::getSingleton('admin/session')->isAllowed('sales/order/actions/edit')
				...
				$block->addButton('edit', array(
					...
					'class'     => 'go',
					'onclick'   => 'setLocation(\''.$block->getUrl('*/editoppcw/index', array('invoice_id'=>$invoice->getId())).'\')'
				));
			}
		}
	}
	...
	{
		if (Mage::registry('oppcw_external_checkout_login') === true) {
	...
	public function collectExternalCheckoutWidgets(Varien_Event_Observer $observer)
	{
		...
		$widgets = array_merge($widgets, Mage::getModel('oppcw/externalCheckoutWidgets')->getWidgets());
		$observer->getEvent()->setWidgets($widgets);
	}

	public function registerTranslationResolver(Varien_Event_Observer $observer)
	{
		if (!Mage::registry('customweb_oppcw_transaction_resolver_registered')) {
			Customweb_I18n_Translation::getInstance()->addResolver(new Customweb_OPPCw_Model_TranslationResolver());
			Mage::register('customweb_oppcw_transaction_resolver_registered', true);
		}
	}

	public function registerLoggingListener(Varien_Event_Observer $observer)
	{
		if (!Mage::registry('customweb_oppcw_logging_listener_registered')) {
			Customweb_Core_Logger_Factory::addListener(new Customweb_OPPCw_Model_LoggingListener());
			Mage::register('customweb_oppcw_logging_listener_registered', true);
		}
	}
	...
	public function migrateCustomerTransactions(Varien_Event_Observer $observer)
	{
		...
		Mage::helper('OPPCw')->migrateCustomersTransactions($customer);
	}
}
