 ...
 * 
 */
class Customweb_OPPCw_Block_Checkout extends Mage_Core_Block_Template {

	protected function _construct(){
		parent::_construct();
		$this->setTemplate('customweb/oppcw/checkout.phtml');
	}
	...
	public function getPaymentMethods(){
		...
		foreach ($payments as $paymentCode => $paymentModel) {
			if (preg_match("/oppcw/i", $paymentCode)) {
	...
	public function getAuthorizeUrl(){
		return Mage::getUrl('OPPCw/process/authorize', array(
			'_secure' => true 
		));
	}
	...
	public function getSaveShippingMethodUrl(){
		return Mage::getUrl('OPPCw/onepage/saveShippingMethod', array(
			'_secure' => true 
		));
	}
	...
	public function getPreloadUrl(){
		if (version_compare(Mage::getVersion(), '1.8', '>=')) {
			return Mage::getUrl('OPPCw/process/preloadOnepage', array(
				'_secure' => true 
			));
		}
		else {
