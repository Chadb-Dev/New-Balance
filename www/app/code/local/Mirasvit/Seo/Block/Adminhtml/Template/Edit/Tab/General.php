...
class Mirasvit_Seo_Block_Adminhtml_Template_Edit_Tab_General extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        if ($model->getRuleType() == Mirasvit_Seo_Model_Config::CATEGORIES_RULE) {
            $descriptionWarning = 'Wiil only be applied if native Magento category description is displayed on the frontend';
        }
        ...
        if ($model && $model->getId()) {
            ...
            if ($model->getRuleType() == Mirasvit_Seo_Model_Config::CATEGORIES_RULE) {
                $fieldset->addField('category_description', 'textarea', array(
                    ...
                    'label'     => Mage::helper('seo')->__('Category Description'),
                    'value'     => $model->getCategoryDescription(),
                    'note'      => $descriptionWarning,
                ));
            }
